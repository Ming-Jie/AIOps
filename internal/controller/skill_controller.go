...
func NewSkillController(skillService *service.SkillService, registry *skills.Registry, skillsDir string, jwtCfg auth.JWTConfig, userStore storage.UserStore) *SkillController {
	return &SkillController{skillService: skillService, registry: registry, skillsDir: skillsDir, jwtCfg: jwtCfg, userStore: userStore}
}

func (c *SkillController) RegisterRoutes(r *server.Hertz) {
	skills := r.Group("/api/v1/skills")
	if c.userStore != nil {
		skills.Use(auth.JWTMiddleware(c.jwtCfg, c.getUserForMiddleware))
	}
...
func (c *SkillController) getUserForMiddleware(userID int64) (*auth.User, error) {
...
func (c *SkillController) currentUser(hc *app.RequestContext) *auth.User {
	return auth.GetCurrentUser(hc)
}

func (c *SkillController) checkSkillOwner(hc *app.RequestContext, skillID int64) bool {
	user := c.currentUser(hc)
	if user == nil || user.IsAdmin {
		return true
	}
	skill, err := c.skillService.GetSkill(skillID)
	if err != nil {
	...
	if skill.CreatedBy == 0 {
		hc.JSON(http.StatusForbidden, schema.ErrorResponse("cannot modify built-in skill"))
		return false
	}
	if skill.CreatedBy != user.ID {
		hc.JSON(http.StatusForbidden, schema.ErrorResponse("you do not have permission to modify this skill"))
		return false
	}
	return true
}
...
func (c *SkillController) ListSkills(ctx context.Context, hc *app.RequestContext) {
	allSkills, err := c.skillService.ListSkills()
	if err != nil {
		hc.JSON(http.StatusInternalServerError, schema.ErrorResponse(err.Error()))
		return
	}
	user := c.currentUser(hc)
	out := make([]*schema.Skill, 0, len(allSkills))
	for _, sk := range allSkills {
		if user != nil && !user.IsAdmin && sk.CreatedBy != 0 && sk.CreatedBy != user.ID {
			continue
		}
		out = append(out, sk)
	}
	hc.JSON(http.StatusOK, schema.SuccessResponse(out))
}
...
func (c *SkillController) CreateSkill(ctx context.Context, hc *app.RequestContext) {
	...
	skill, err := c.skillService.CreateSkill(&req, user.ID)
	if err != nil {
		hc.JSON(http.StatusInternalServerError, schema.ErrorResponse(err.Error()))
		return
	}
